...
# from configs.configs import current_band, num_head
...
class EntropyWeighting(nn.Module):
    ...
    def initialize_positional_encoding(self, c, h, w,device):
        self.positional_encoding = torch.zeros(1, c, h, w).to(device)
        self.initialized = True

    def forward(self, x):
        b, c, h, w = x.size()
        if not self.initialized or self.positional_encoding.size() != (1,) + x.size()[1:]:
            self.initialize_positional_encoding(c, h, w, x.device)
...
class FocusedLinearAttention(nn.Module):
    def __init__(self,device, dim, num_patches, num_heads=5, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(size=(1, num_patches // (sr_ratio * sr_ratio), dim))).to(device)
...
class AIWSEN(nn.Module):
    def __init__(self,device, inchannel , num_head, layers=[1,1,1,1],img_size=(7,7), embed_dims=[96,192,384,768],
        ...
        self.lstm1 = RNN(inchannel, 32)
        self.lstm2 = RNN(768, 256)
        self.patch_embed = PatchEmbed(
            patch_size=in_patch_size, stride=in_stride, padding=in_pad,
            in_chans=inchannel, embed_dim=embed_dims[0])

        self.FLA = FocusedLinearAttention(device,
                dim=inchannel, num_patches=49,
                num_heads=num_head, qkv_bias=qkv_bias, qk_scale=qk_scale,
