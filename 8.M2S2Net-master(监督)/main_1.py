...
def visualize_predictions_with_tpfp(labeled_y, pre_u, labeled_index, full_y, height, width, dataset, output_path, TN,
                                    FP, FN, TP):
    """
    ...
    # # 处理标记区域
    # # 注意: 对于BayArea/Barbara，变化=1，未变化=2 → 转换为: 变化=1，未变化=0  2未标记
    # # 其他数据集: 变化=1，未变化=0
    # if dataset in ['BayArea', 'Barbara']:
    #     true_binary = true_img.copy()
    #     # 创建掩码，标记出值为0和2的像素
    #     mask_0 = true_binary == 0
    #     mask_2 = true_binary == 2
    #     # 先将2变为0
    #     true_binary[mask_2] = 0
    #     # 再将原来的0变为2
    #     true_binary[mask_0] = 2
    # else:
    #     true_binary = true_img.copy()
    # 处理未标记区域（仅BayArea和Barbara数据集有）
    if dataset in ['BayArea', 'Barbara']:
...
def train_and_test(args, experiment_num, dataset_data, train_loader):
    ...
    all_x, all_y, labeled_index, height, width, full_y = dataset_data
    ...
    # 分割验证集
    train_indices = np.random.choice(len(train_loader.dataset), int(len(train_loader.dataset) * 0.8), replace=False)
    val_indices = np.array([i for i in range(len(train_loader.dataset)) if i not in train_indices])
    ...
    train_loader_split = DataLoader(train_subset, batch_size=args.batch_size, shuffle=True, 
                                  num_workers=args.num_workers, pin_memory=True)
    val_loader = DataLoader(val_subset, batch_size=args.batch_size, shuffle=False,
                          num_workers=args.num_workers, pin_memory=True)

    print(f"[Info]: 数据已分割！训练样本: {len(train_subset)}, 验证样本: {len(val_subset)} 实验次数: {experiment_num}")
    ...
    for epoch in range(args.epoches):
        ...
        train_acc, train_loss, _, _ = train_epoch(model, train_loader_split, criterion, optimizer, device)
    ...
    print(f"训练完成！最佳模型(epoch {best_epoch})已保存至: {model_path}")
...
def get_args_parser():
    ...
    parser.add_argument('--datasets', choices=['farmland', 'Barbara', 'BayArea'],
