...
train_numbers = [0.001, 0.005, 0.01, 0.02, 0.03, 0.05, 0.1, 0.2]
datasets = ['BayArea', 'Barbara', 'farmland', 'river', 'hermiston']
results = {}
...
for dataset in datasets:
    ...
    for train_number in train_numbers:
        args.train_number = train_number
        print(f"Training with train_number: {train_number}")

        selected_uc = np.random.choice(uc_position.shape[0], int(train_number * uc_position.shape[0]), replace = False)
        selected_c = np.random.choice(c_position.shape[0], int(train_number * c_position.shape[0]), replace = False)
        print(int(train_number * uc_position.shape[0]))
        print(int(train_number * c_position.shape[0]))
        selected_uc_position=uc_position[selected_uc]
        selected_c_position=c_position[selected_c]
        TR = np.zeros(data_label.shape)
        for i in range (int(train_number*c_position.shape[0])):
            TR[selected_c_position[i][0],selected_c_position[i][1]]=1
        for i in range (int(train_number*uc_position.shape[0])):
        ...
        print("**************************************************")
        print("Parameter:")
        torch.save(model.state_dict(), f"log/hygstan_{args.dataset}_{train_number}.pth")

        oa_results.append(OA2)
    ...
    # 绘制图表
    plt.figure()
    plt.plot(train_numbers, oa_results, marker='o')
    plt.xlabel('Train Numbers')
    plt.ylabel('Test OA')
    plt.title(f'{dataset} - Train Numbers vs Test OA')
    plt.grid(True)
    plt.savefig(f'{dataset}_train_numbers_vs_OA.png')
    plt.close()
