...
def loadData(names):
    ...
    if names == 'BayArea':
        ...
        data_label = loadmat(r'Z:\pycharm\pythonProject\1.SSA\dataset\bayArea\bayArea_gtChanges2.mat.mat')['HypeRvieW']


    return  data_t1, data_t2, data_label


def set_train_sample_by_ratio(X, Y, train_ratio=0.1):
    # 依据标签进行分层抽样
...
def normalize(X, type):
    ...
    if type == 1:   #均值和标准差的归一化
        for i in range(X.shape[2]):
            temp = X[:, :, i]
            mean = np.mean(temp)
            std = np.std(temp)
            x[:, :, i] = ((temp - mean) / std)
    if type == 2:   #Min - Max 归一化
        for i in range(X.shape[2]):
            min = np.min(X[:, :, i])
            max = np.max(X[:, :, i])
            scale = max - min
...
def reports(test_iter, net, device):
    ...
    return classification, confusion, oa, aa, kappa, f1, auc


def generater(X_1, X_2, Y, batchsize, windowSize):
    alldataX1, alldataY = create_patches(X_1, Y, window_size=windowSize)
    alldataX1 = np.transpose(alldataX1, (0, 3, 1, 2))   # B C H W  ，这里的H W = window_size
    X1_train, X1_test, y_train, y_test = set_train_sample_by_ratio(alldataX1, alldataY)
    ...
    X2_train, X2_test, y_train, y_test = set_train_sample_by_ratio(alldataX2, alldataY)
...
def weight_init(layer):
    ...
    elif isinstance(layer, torch.nn.BatchNorm2d):    #初始化为权重 1.0、偏置 0.0 意味着一开始这个线性变换是恒等变换，不会改变归一化后的数据
...
def binary(x, th, bathsize):   #根据给定的阈值 th 对输入张量 x 进行二值化处理，将张量中的元素转换为 0 或 1 这两种值
