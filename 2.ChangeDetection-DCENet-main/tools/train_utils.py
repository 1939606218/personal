...
def adjust_lr_sub(lr_init, lr_gamma, optimizer, epoch, step_index):
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr    #循环遍历优化器的参数组（param_groups），将每个参数组中的学习率设置为刚刚计算得到的lr值

    return lr


def train_ULiter(trl_data, tru_data, model, optimizer, device, cfg):
    torch.autograd.set_detect_anomaly(True)    #设置自动梯度异常检测
    ...
    batch_num = math.ceil(len(trl_data) / batch_size_l) - 1  # math.ceil 向上取整 , 计算出的批次数量,减 1 的原因不清楚
    print('Sample: label{}/{} || unlabel: {}/{} ;batch_num: {}'.format(len(trl_data), batch_size_l, len(tru_data),
                                                                       batch_size_u, batch_num))

    amp_cm = autocast if 1 else contextlib.nullcontext   #设置自动混合精度上下文管理器
    ...
    for epoch in range(start_epoch + 1, epoch_size):
        ...
        batch_datau = DataLoader(tru_data, batch_size_u, shuffle=True, num_workers=num_workers, pin_memory=True,
                                 drop_last=True)
        #是否打乱数据顺序（shuffle=True，有助于在训练中让模型看到不同顺序的数据，增强模型的泛化能力）、数据加载的工作线程数量（num_workers）、
        #是否将数据固定到内存（pin_memory=True，可以加快数据传输到 GPU 等设备的速度）
        # 是否丢弃最后一个不完整的批次（drop_last=True，保证每个批次的数据量固定）等参数，方便后续按批次获取训练数据
        if lr_adjust:
        ...
        for batch_idx, data in enumerate(zip(batch_datal, batch_datau)): #有标签数据加载器和无标签数据加载器中的批次数据通过zip函数进行组合，同时获取批次索引batch_idx和对应批次的数据
            ...
            with amp_cm():
                # sup+CL+KL
                logits_x_l, logits_x_u, L_cl, L_unsup = model(xl1, xl2, xu1, xu2)  #有标签数据的预测结果（logits_x_l）、无标签数据的预测结果（logits_x_u）
            ...
            # batch_time = time.time() - batch_time0   #当前批次的训练耗时（batch_time）
            # batch_eta = batch_time * (batch_num - batch_idx)  #通过当前批次耗时乘以剩余批次数量（batch_num - batch_idx）来估计本批次到本轮次训练结束还需要的时间（batch_eta)
            # epoch_eta = int(batch_time * (epoch_size - epoch) * batch_num + batch_eta) #预估全部训练结束时间
    ...
    save_model = dict(
        model=model.state_dict(),#获取模型当前的所有参数（以字典形式返回，字典的键是参数名称，值是对应的参数张量）
        epoch=epoch_size
    )
