...
class Gaussnoise3D(nn.Module):
    def __init__(self, mean=0, stddev=0.1): #mean表示高斯噪声的均值，默认值为 0；stddev表示高斯噪声的标准差，默认值为 0.1
    ...
    def forward(self, x):
        ...
        for i in range(x.shape[-1]):
            ...
            x_noisy[:, :, :, i] = x[:, :, :, i] + noise  #在每个通道上独立地添加噪声,加在第i个通道
        return x_noisy
...
class DCENet(nn.Module):
    ...
    def __init__(self, in_fea_num, num_classes=2):
        ...
        self.getdiff = DFEDSubnet(in_fea_num)   #DFA模块
        self.enhence = Gaussnoise3D()
        self.downa = DownEncoder()
        self.downb = DownEncoder()  #三次下采样
    ...
    def test(self, xl1, xl2):
        ...
        down = down.squeeze(2).squeeze(2)   #对down张量的第 3 个维度进行压缩,如果这个维度的大小为 1，那么这个维度就会被去除。
        out = self.fc(down)
    ...
    def forward(self, xl1, xl2, xu1, xu2):
        if self.training:
            ...
            return outl, outu, CLloss, klloss  #outl, outu都是原始序列的概率密度输出
...
if __name__ == '__main__':
    ...
    # 创建模型实例
    model = DCENet(num_channels)

    # 创建输入数据
    xl1 = torch.randn(batch_size, num_channels, patch_size, patch_size)
    xl2 = torch.randn(batch_size, num_channels, patch_size, patch_size)
    xu1 = torch.randn(batch_size, num_channels, patch_size, patch_size)
    xu2 = torch.randn(batch_size, num_channels, patch_size, patch_size)

    # 计算 FLOPs 和参数数量
    flops, params = profile(model, inputs=(xl1, xl2, xu1, xu2))
