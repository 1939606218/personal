...
# def normalization(X):
#     """
#     对每个波段分别进行 Min-Max 归一化，将其映射到 [0, 1] 区间
#     :param X: 输入数据，形状为 (H, W, C)，C 表示波段数
#     :return: 每个波段归一化后的数据
#     """
#     H, W, C = X.shape
...
#
#     for i in range(C):
#         band = X[:, :, i].reshape(-1, 1)  # 拉成一列
#         scaler = MinMaxScaler()
#         band_scaled = scaler.fit_transform(band)
#         X_norm[:, :, i] = band_scaled.reshape(H, W)
#
#     return X_norm
...
def generater(X_1, X_2, Y, batchsize, train_ratio, device, windowSize, noise_std=0.1):
    # 生成数据块
    alldataX1, alldataY1 = create_patches(X_1, Y, window_size=windowSize)
    alldataX1 = np.transpose(alldataX1, (0, 3, 1, 2))  # (N, C, H, W)
    ...
    idx_train, idx_test, y_train, y_test = train_test_split(
        ...
        train_size=train_ratio,
        stratify=alldataY1,  # 关键修改：按标签分层抽样
        random_state=42
    )

    # 根据索引划分数据
    X1_train, X1_test = alldataX1[idx_train], alldataX1[idx_test]
    X2_train, X2_test = alldataX2[idx_train], alldataX2[idx_test]
    ...
    X1_all = alldataX1[mask_all]
    X2_all = alldataX2[mask_all]
    Y_all = alldataY1[mask_all]
    all_position_indices = np.arange(height * width)[mask_all]
