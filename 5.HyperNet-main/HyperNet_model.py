...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False)     #填充  不改变特征图大小
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)  #填充，不改变特征图大小
        self.sigmoid = nn.Sigmoid()
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.cbam is None:               #只有第一个RSAB配备了下采样
            out = self.cbam(out)
        out += residual
        out = self.relu(out)
...
class HyperNet(nn.Module):
    def __init__(self, block, layernum, gamma):
        ...
        self.conv1 = self._make_layer(block, layernum[0], layernum[1])   #RSAB  第一次下采样，后两次不下采样
        self.conv2 = self._make_layer(block, layernum[1], layernum[1])
        self.conv3 = self._make_layer(block, layernum[1], layernum[1])
        self.fc1 = FC(layernum[0], layernum[1])            #RCAB
    ...
    def spatial(self, x):
        a1 = self.conv1(x)
        a1 = self.conv2(a1)
        a1 = self.avgpool(self.conv3(a1))     # #RSAB*3
        return a1
    def sprectral(self, x):
        b1 = self.fc3(self.fc2(self.fc1(x)))    #RCAB*3
        return b1
...
class Focal_cos(nn.Module):
    ...
    def forward(self, p1, p2, z1, z2, idx):
        ...
        loss1 = torch.mul(torch.pow((2 - cos_), self.gamma), cos_)    #元素乘法
        cos_ = self.cos(p2, z1.detach())  # [H*W, 1]                    #z1.detach()的主要作用是返回一个新的张量，这个新张量和z1共享相同的数据，但它不会在计算图中被追踪
...
if __name__ == "__main__":
 ...
 #    print(f1.shape)
     ...
     layernum = [128, 64, 128, 64]
     gamma = 2
     model = HyperNet(BasicBlock, layernum, gamma)
     ...
     # 定义输入数据
     x1 = torch.randn(64, 128, 5, 5).to(device)
     x2 = torch.randn(64, 128, 5, 5).to(device)
     idx = torch.tensor([1, 2, 3, 4]).to(device)

     # 使用 thop 计算模型的 FLOPs 和参数数量
     flops, params = profile(model, inputs=(x1, x2, idx))
